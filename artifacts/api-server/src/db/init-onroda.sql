...
-- Allgemeine Hilfe-Anfragen (Tab Hilfe, Migration 065)
...
CREATE INDEX IF NOT EXISTS app_help_tickets_passenger_created_idx ON app_help_tickets (passenger_id, created_at DESC);
CREATE INDEX IF NOT EXISTS app_help_tickets_status_created_idx ON app_help_tickets (status, created_at DESC);
CREATE INDEX IF NOT EXISTS app_help_tickets_email_created_idx ON app_help_tickets (passenger_email, created_at DESC);
...
CREATE TABLE IF NOT EXISTS homepage_content (
  ...
  services_kicker TEXT NOT NULL DEFAULT '',
  services_title TEXT NOT NULL DEFAULT '',
  services_subline TEXT NOT NULL DEFAULT '',
  services_cards JSONB NOT NULL DEFAULT '[]'::jsonb,
  manifest_kicker TEXT NOT NULL DEFAULT '',
...
-- App / Betrieb: zentrale App-Regeln + Einfahrt-Gebiete (Migration 049, für frische DB gespiegelt)
...
-- Medical-Modul Phase 1 (Migration 075): Partner-IK + Fall/Dokument/Ampel-Review.
ALTER TABLE admin_companies
...
-- Marketing-Homepage: anonyme Besucherstatistik (Migration 097)
